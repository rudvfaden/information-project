...
\usepackage{fixltx2e} 									% fix common latex errors
%% Math
...
\end{align}
is non decreasing in \(s\) for all \(x\).\footnote{Note that \cref{eq:acuracy tranformation} can also be written as \(F^{\eta'}(T_{\eta,s}(x)\mid s)=F^{\eta}(x\mid s)\)}
...
I now turn to the problem of ordering information over time. Let \(\eta(i)\) be the information order at time \(i\) after receiving \(\pmb{X}^{\eta(i)}=X^{1},\ldots,X^{i}\) signals. It is still assumed that \(\{\pmb{X}^{\eta(i)}\}_{\eta(i)\in E}\) and that higher \(\eta(i)\) implies higher accuracy.
...
\begin{assumption}
	\(\eta(i)\) is continuous and differential in \(i\)
\end{assumption}
\begin{thm}\label{thm:mri}
For a A-ordered family of signals the \(MR(i)\geq0\) iff the T-transformation function \(T_{\eta(i),s}\) is increasing in \(i\)
\end{thm}
\begin{proof}
I want to show that
\[
	\pder{i}\int_X\int_S u(s,t^{\eta(i)}(x),s)\, dG^{\eta(i)}\, dF(x)\geq0
\]
Given the definition of the T-function, I can write the above as
\[
	\pder{i}\int_X\int_S u(s,t^{\eta(i)}(T_{\eta(i),s}(x)),s)\, dG^{\eta(i-1)}\, dF(x)
\]
...
\[
	\int_S \left[\pder{t}u(t^{\eta(i)}(T_{\eta(i),s}(x)),s)\right]\pder{\eta(i)}t^{\eta(i)}(x)\pder{i}T_{\eta(i),s}(x)\, dG^{\eta(i-1)}(s\mid x)
\]
However the first-order-condition says that
\[
	\int_S \left[\pder{t}u(t^{\eta(i)}(T_{\eta(i),s}(x)),s)\right]\pder{\eta(i)}t^{\eta(i)}(x)dG^{\eta(i-1)}(s\mid x)=0
\]
...
\begin{defn}\label{def:i-accuracy-order}
