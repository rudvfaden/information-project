...
To do:
\begin{enumerate}
\item Show that utility increases with information

\begin{enumerate}
\item Show that $u(t,s)f(s\mid x)$ is increasing in $x$

\begin{enumerate}
\item show that the greatest lower bound of $u(t,s)f(s\mid x)\ge u(t,s)f(s\mid x')\forall t\in T$
where $x>x'$ implies that $f(s\mid x)$ is more accurate than $f(s\mid x')$
\end{enumerate}
\end{enumerate}
\item Define how information is accumulated over time
\item Show that if information is accumulated over time, such that $f_{t}(s\mid x)$
is more accurate than $f_{t-1}(s\mid x)$, then $u_{t}(t,s)f_{t}(s\mid x)\ge u_{t-1}(t,s)f_{t-1}(s\mid x)\forall t\in T$
\item Show that given 1-3 switching cost are increasing over time

\begin{enumerate}
\item Let the accuracy of $f(s\mid x)$ depend on physician skill $y$ and
let $y$ be distributed with $G(y)$, then show that the expected
value of $u_{t}(t,s)f_{t}(s\mid x)G(y)$ is decreasing in $t$
\end{enumerate}
\end{enumerate}
Further it might be interesting to show that 
\begin{enumerate}
\item $\phi(x)=\argmax_{x} u(t,s)f(s\mid x)$ is increasing in $x$
\item $\phi(t)$\end{enumerate}
\end{comment}
...
\begin{equation}
u(t,\theta,\gamma)=\mathbb{E}[u(t,s,x)]=\max_{\gamma}\left[\int_{X}\max_{t}\int_{a}^{b}u(t,s)dF^{\gamma}(x\mid s)dF^{\gamma}(x)dsdx-C(\gamma)\right]\label{eq:expected-posterior}
\end{equation}


now the first order conditions of expected utility gives that 
\begin{eqnarray*}
\int_{X}\int_{a}^{b}u(t,s)\left[f(s\mid x,\gamma)f_{\gamma}(x,\gamma)+f_{\gamma}(s\mid x,\gamma)f(s,\theta)\right]dsdx & = & C_{\gamma}(\gamma)\\
\int_{X}\int_{a}^{b}u_{t}(t,s)f(s\mid x,\gamma)f(x,\gamma)dsdx & = & 0
\end{eqnarray*}
...
shifted to the right by the distance $\Delta$ for all $s$. Then
I can write 
\[
u(t,\theta,\gamma)=\mathbb{E}[u(t,s,x)]=\max_{\gamma}\left[\int_{X}\max_{t}\left\{ \int_{a}^{b}u(t,s)f(s\mid x,\gamma)ds\right\} f(x,\gamma)dx-C(\gamma)\right]
\]
